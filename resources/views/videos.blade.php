...
@foreach($searchResults['body']['data'] as $videoData)
    <h2>{{ $videoData['name'] }}</h2>
        <iframe src="https://player.vimeo.com/{{ preg_replace("/videos/","video",$videoData['uri']) }}?badge=0&autopause=0&player_id=0" width="420" height="360" frameborder="0" title="{{ $videoData['name'] }}" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>
@endforeach
</div>